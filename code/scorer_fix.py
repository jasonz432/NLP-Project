...
class FleschScorer:
    ...
    def __init__(self, name_module, flesch_score, device="cuda"):
    ...
    def score(self, summaries, index):
        ...
        for text in summaries:
            try:
                flesch_scores.append(get_flesch(text))
            except:
                flesch_scores.append(100)
        flesch_scores = [1 - (abs(fs - self.flesch_score) / 100) for fs in flesch_scores]

        return torch.tensor(flesch_scores).to(self.device)


# class BERTandFleschScoreScorer:
#     """
#     Scorer using BS-Fact, code adapted from bertscore official repo: https://github.com/Tiiiger/bert_score
#     """

#     def __init__(self, model_name="roberta-large", device="cuda", num_layers=17, cache_dir=".cache", flesch_score=50,
...
#         This is called each iteration only once to save computation.
#         """
#         self.bertscore_input_embedding, self.bertscore_input_attention_mask, self.bertscore_input_idf = self.encode_text(
#             input_str)
...
#         flesch_scores = torch.tensor(flesch_scores).to(self.device)
#         assert flesch_scores.size() == bertscore_scores.size()

#         # import pdb
#         # pdb.set_trace()

#         return self.readability_weight * flesch_scores + (1 - self.readability_weight) * bertscore_scores
